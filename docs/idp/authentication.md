...
| Same-email linking                      | `account.accountLinking`, implicit verified-email linking, `linkSocial`                                                                | Google is the only configured social/linking provider; local and provider emails must be verified. Different-email linking remains disabled.                                                                                                                                                   |
...
| Sessions and sign-out                   | `getSession`, `revokeSession(s)`, `signOut`                                                                                            | `databaseHooks.session.create.before` rejects disabled users. An admin status change to `disabled` deletes that user's persisted sessions in the same transaction as the user update, so existing session cookies no longer authenticate; unrelated users' sessions are not touched.           |
...
- Credential-created users must verify their email before sign-in creates a session.
- A verified Google identity may implicitly link only to the same normalized, already-verified local
  email. Google does not overwrite local profile, role, or status fields. No provider-name trust
...
- Google first access for a valid invitee creates one active user with the invitation role. The
  invitation is accepted after Better Auth completes transactional user/account creation.
