...
test("previews the neutral authenticated shell without product-domain navigation", async ({
  ...
  await expect(page.getByRole("link", { name: "Usuários", exact: true })).toHaveAttribute(
    "href",
    "/users",
  )
...
test("keeps desktop persistence, mobile navigation, themes, and focus behavior", async ({
  ...
  await trigger.click()
  const dialog = await page.getByRole("dialog", { name: "Navegação do workspace" })
  await expect(dialog.getByRole("link", { name: "Usuários" })).toBeVisible()
