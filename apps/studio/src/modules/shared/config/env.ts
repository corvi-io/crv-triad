...
const publicEnvSchema = z.object({
  ...
  VITE_POSTHOG_HOST: z.string().url().default("http://localhost:8000/e"),
  VITE_POSTHOG_KEY: z.string().trim().default(""),
  VITE_POSTHOG_REPLAY_SAMPLE_RATE: z.coerce.number().min(0).max(1).default(0.1),
  VITE_RELEASE: z.string().trim().default("development"),
...
export const env = {
  ...
  posthogHost: parsedEnv.VITE_POSTHOG_HOST,
  posthogKey: parsedEnv.VITE_POSTHOG_KEY,
  posthogReplaySampleRate: parsedEnv.VITE_POSTHOG_REPLAY_SAMPLE_RATE,
  release: parsedEnv.VITE_RELEASE,
