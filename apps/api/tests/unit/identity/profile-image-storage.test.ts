...
describe("local profile image storage", () => {
  it("persists, serves, and deletes an owned image", async () => {
    ...
    await expect(
      storage.put({ body, contentType: "image/png", key: "../another-user/image.png" }),
    ).rejects.toThrow("Invalid profile image object key.")
  ...
  it("stores and deletes private objects through the R2 adapter", async () => {
    ...
    expect(storage.publicUrl("image.webp")).toBe(
      "https://api.example.test/profile-images/image.webp",
    )
