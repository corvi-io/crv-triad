...
import { createAuthEmailSender } from "../../../src/modules/idp/identity/transactional-email.js"
...
describe("transactional authentication email", () => {
  it("sends invitation email through the shared transport", async () => {
    ...
    expect(requests[0]?.body).toContain("A rotina da barbearia começa por aqui.")
    expect(requests[0]?.body).toContain("Sistema operacional para barbearias")
